...
namespace TuningWebApp.Controllers
{
    public class TuningDatabaseHandler
    {
        ...
        public void UpdateTuningVariant(InputNewVariant inputNewVariant)
        {
            ...
            string deleteQuery3 = "DELETE from [AvailableSolution] where [TuningSpecialInfoId] =@newTuningVariantId";
            string deleteQuery4 = "DELETE from [TuningSpecialInfo] where [Id] =@newTuningVariantId";
            string deleteQuery5 = "DELETE from [TuningVariant] where [TuningId] =@newTuningVariantId";

            using (var con = new SqlConnection(_configuration.GetConnectionString("dbo")))
            {
                // Open the connection
                con.Open();
                con.Execute(deleteQuery, new { newTuningVariantId = inputNewVariant.tuningvariantid });
                con.Execute(deleteQuery2, new { newTuningVariantId = inputNewVariant.tuningvariantid });
                con.Execute(deleteQuery3, new { newTuningVariantId = inputNewVariant.tuningvariantid });
                con.Execute(deleteQuery4, new { newTuningVariantId = inputNewVariant.tuningvariantid });
                con.Execute(deleteQuery5, new { newTuningVariantId = inputNewVariant.tuningvariantid });
            }

            CreateNewTuningVariant(inputNewVariant.tuningvariantid, inputNewVariant.TypeName, $"{inputNewVariant.YearStart}-{inputNewVariant.YearEnd}", inputNewVariant.EngineName, inputNewVariant.EnginePowerKw.ToString(), inputNewVariant.FuelVariant);
            CreateNewTuningSpecialInfo(inputNewVariant.tuningvariantid, inputNewVariant.SpecialInfo, inputNewVariant.SelectedEcu.Id);
            foreach (var solution in inputNewVariant.AvailableSolutions)
            {
                CreateNewAvailableSolution(inputNewVariant.tuningvariantid, solution.Name, solution.Information, solution.Value1, solution.Value2);
        ...
        private void CreateNewTuningSpecialInfo(string newTuningVariantIdSpecial, string specialInfo, int ecuId)
        {
            ...
            string query = @"
                             INSERT INTO [dbo].[TuningSpecialInfo] (Id, AdditionalInformation, EcuInfoId, isDeleted)
                                VALUES (@Id, @AdditionalInformation, @EcuInfoId , 0)";
        ...
        private void CreateNewTuningVariant(string newTuningVariantId, string typeName, string year, string engineName, string horsepower, string fuelVariant)
        {
            ...
            string query = @"
                              ...
                              VALUES (@TuningId, @TypeName, @Year, @Engine, @Horsepower, @Variant, 0)";
        ...
        private void CreateNewAvailableSolution(string newTuningVariantIdSpecial, string name, string information, int value1, int value2)
        {
            ...
            string query = @"
                             ...
                             VALUES (@TuningSpecialInfoId, @Name, @Information, @Value1, @Value2, @Checked, 0)";
